...
// El orden de estas columnas tiene que coincidir EXACTO con la fila 1 del Sheet.
// "formato" se agregó al final (columna 26) para no correr las columnas ya cargadas.
export const COLUMNAS_PRODUCTOS = [
  ...
  'esVariante', 'varianteDeId', 'motivo', 'sinNiveles', 'mediosDePagoJSON', 'archivado', 'formato'
];

// Deriva un formato inicial razonable a partir de la modalidad vieja, para los productos que
// todavía no tienen "formato" cargado explícitamente (columna vacía).
function formatoPorDefecto(modalidad) {
  if (modalidad === 'On demand' || modalidad === 'Ebook' || modalidad === 'Comunidad') return 'Asincrónico';
  return 'Sincrónico';
}
...
export function parseProducto(row) {
  return {
    ...
    formato: row.formato || formatoPorDefecto(row.modalidad),
