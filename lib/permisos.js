...
// Comunidades: por ahora solo Diego puntualmente, más quien ya tenga acceso a Estudiantes
// (sección nueva, todavía "Próximamente" — se va a abrir de a poco).
export function tienePermisoComunidades(usuario) {
  if (!usuario) return false;
  if ((usuario.email || '').trim().toLowerCase() === 'diegolernerdl@gmail.com') return true;
  return tienePermisoEstudiantes(usuario);
}
...
// Productos y Valores: cualquiera logueado puede VER los precios. Editar (cargar valores,
// descuentos, links de pago) queda solo para Admin + Macarena puntualmente — no es un permiso
// por rol porque otras personas comparten el rol Coordinador con ella pero no deben editar esto.
const EMAILS_PRODUCTOS_EDITAR = ['macarena.juncos@institutoilce.com'];
export function tienePermisoProductosVer(usuario) {
  return !!usuario;
}
export function tienePermisoProductosEditar(usuario) {
  return !!usuario && (usuario.roles?.includes('Admin') || EMAILS_PRODUCTOS_EDITAR.includes((usuario.email || '').trim().toLowerCase()));
}
