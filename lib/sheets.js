...
export async function readSheetCola(tabName, maxFilas = 3000) {
  ...
  const totalFilas = meta.data.sheets?.[0]?.properties?.gridProperties?.rowCount || 0;

  const [headerRes, datosRes] = await Promise.all([
    sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${tabName}!1:1` }),
    sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: `${tabName}!${Math.max(2, totalFilas - maxFilas + 1)}:${totalFilas}`
    })
  ]);

  const headers = headerRes.data.values?.[0] || [];
  const rows = datosRes.data.values || [];
  const offsetFilaReal = Math.max(2, totalFilas - maxFilas + 1);

  return rows.map((row, idx) => {
    const obj = { _rowIndex: offsetFilaReal + idx };
    headers.forEach((h, i) => {
      obj[h] = row[i] ?? '';
    });
    return obj;
