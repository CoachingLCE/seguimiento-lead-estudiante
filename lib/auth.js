...
export async function findUsuario(email) {
  ...
  return {
    email: match.Email,
    nombre: match.Nombre,
    // Se separa por coma (formato real que guarda la app), pero también tolera "+" por si alguien
    // edita la columna a mano en el Sheet copiando el formato que se ve en pantalla (ej: "Coordinador + Academico").
