...
// Mail semanal con el resumen de "Fichas enviadas" (mismos datos que /fichas-enviadas): leads
// que recibieron la ficha de inscripción y todavía no compraron.
export async function enviarMailResumenFichasEnviadas(destinatarios, fichas) {
  const transporter = getTransporter();
  await enviarYRegistrar(transporter, 'Resumen semanal de fichas enviadas', {
    from: `Instituto ILCE <${process.env.GMAIL_SENDER_EMAIL}>`,
    to: destinatarios.join(', '),
    subject: `📄 Resumen semanal de fichas enviadas — ${fichas.length} sin comprar`,
    html: htmlResumenFichasEnviadas(fichas)
  });
}
